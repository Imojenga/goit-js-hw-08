...
const createCardsTemplate = images
  .map(image => createGalleryCard(image))
  .join('');

const imgGallery = document.querySelector('.gallery');
imgGallery.insertAdjacentHTML('afterbegin', createCardsTemplate);
...
imgGallery.addEventListener('click', selectCard);
...
function selectCard(event) {
  ...
  const imageInfo = images.find(el => el.original === imageLink);
